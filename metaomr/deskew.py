...
def get_column_offsets(page):
    strip_offsets = get_strip_offsets(page)
    column_offsets = np.zeros(page.orig_size[1], int)
    column_offsets[:strip_size/2] = strip_offsets[0]
    for s in xrange(strips.shape[1] - 1):
