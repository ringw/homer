...
def staff_barlines(page, staff_num):
    img_slice = bitimage.as_hostimage(page.staves.extract_staff(staff_num,
                                                page.barline_filter,
                                                extract_lines=8))
    staff_dist = page.staves.staff_dist[staff_num]
    ...
    staff_proj = img_slice[staff_dist*2:staff_dist*6, :].sum(0)
    ...
    is_barline = staff_proj > staff_dist * 4 * 0.8
    is_background = gap_proj <= staff_dist
    ...
    for label in xrange(1, num_labels+1):
        ...
        if pos[-1] - pos[0] > staff_dist * 1.5:
            continue
        left_bg = is_background[max(0, pos[0] - staff_dist/2) : pos[0]]
    ...
    if len(barlines):
        ...
        if barlines[0, 0] - staff[0,0] > staff_dist*2:
            barlines = np.concatenate([[[staff[0,0], staff[0,0]]], barlines])
        if staff[1,0] - barlines[-1, 1] > staff_dist*2:
